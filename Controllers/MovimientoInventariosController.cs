...
namespace Sistema_Gestion_Inventario.Controllers
{
    ...
    public class MovimientoInventariosController : BaseController
    {
        ...
        public MovimientoInventariosController(ApplicationDbContext context, UserManager<ApplicationUser> userManager) // <- cambiado
        {
            _context = context;
        ...
        {
            if (m.Motivo == 1) // Compra
            {
                m.Tipo = "IN"; // normalizo siempre para Compra
                if (m.IdOrdenCompraDetalle == null)
                {
                    ModelState.AddModelError("IdOrdenCompraDetalle",
                        "Cuando el motivo es Compra, debe vincular una línea de Orden de Compra.");
                }
            }
            ...
            if (!ModelState.IsValid)
            {
                ...
                return View("~/Views/MovimientoInventarios/Create.cshtml", m);
            }

            try
            {
                m.IdUsuarioOperador = _userManager.GetUserId(User);
                m.FechaMovimiento = DateTime.UtcNow;
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            return View("~/Views/MovimientoInventarios/Edit.cshtml", m);
        }
        ...
        public async Task<IActionResult> Edit(int id, [Bind("IdMovimiento,IdProducto,IdAlmacen,Tipo,Motivo,Cantidad,CostoUnitario,IdOrdenCompraDetalle,FechaMovimiento,Referencia,IdUsuarioOperador,Observaciones")] MovimientoInventario m)
        {
            ...
            try
            {
                var current = await _context.MovimientoInventario
                    .AsNoTracking()
                    .FirstOrDefaultAsync(x => x.IdMovimiento == id);
                if (current == null) return NotFound();

                m.FechaMovimiento = current.FechaMovimiento;
                m.IdUsuarioOperador = _userManager.GetUserId(User);     // operador actual
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            if (m == null) return NotFound();
            return View("~/Views/MovimientoInventarios/Delete.cshtml", m);
        }
